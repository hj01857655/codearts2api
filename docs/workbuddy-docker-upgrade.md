# WorkBuddy 任务提示词：codearts2api Docker 升级到 v0.1.7
...
把腾讯云轻量实例上现有的 **Docker 部署** codearts2api 原地升级到 `v0.1.7`：
...
| 可用 Release | `v0.1.0`（旧）、`v0.1.1`、`v0.1.2`、`v0.1.3`、`v0.1.4`、`v0.1.5`、`v0.1.6`、**`v0.1.7`（本次目标）**；`v0.1.2` 起带 Docker 版本注入，`v0.1.3` 起带新控制台面板，`v0.1.5` 起修掉一批模型发现与面板读数缺陷，`v0.1.7` 起容器内支持在线更新 |

现状问题（本次要修的）：

- 现有代码是旧版：升级前那份 `Dockerfile` 构建时**未注入版本号**，所以容器里 `codearts2api -version` 永远报 `dev`、`/status` 的 `version` 字段为空
  （该缺陷已在 `v0.1.2` 修复：`Dockerfile` 新增 `VERSION`/`COMMIT`/`BUILD_DATE` 构建参数，`docker-compose.yml` 透传。**`v0.1.1` 及更早不含此修复**；本次升到 `v0.1.7` 同样包含它）
...
- 如果 `origin` 指向的是上游 `HITZY2002/codearts2api` 而不是 `hj01857655/codearts2api`，则需要增加远端后再拉取——`v0.1.7` 只存在于 `hj01857655` 这个 fork 上：
  ```bash
...
```bash
# 4.1 容器内二进制版本（期望输出 v0.1.7，含 commit 与构建时间）
...
| 检查项 | 期望结果 |
| --- | --- |
| `codearts2api -version` | `codearts2api v0.1.7 (commit ..., built ...)`（不再是 `dev`） |
| `docker compose ps` | 容器 `Up`，7866 映射正常 |
| `/status` 的 `version` | 不再是空值，为 `v0.1.7` |
...
| `/admin/api/update/check` | `"supported": true`（`v0.1.7` 起容器内支持在线更新；刚发版时 `has_update` 应为 `false`，见下方说明） |

**关于「检测更新」的预期结果，务必注意**：`v0.1.7` 起**容器内支持在线更新**——更新替换的是容器可写层里的二进制，退出后由 `restart: unless-stopped` 拉起，新文件仍在（做法对齐 sub2api）。因此 `supported` 应为 `true`；若返回「不支持」反而是异常，原样贴 JSON 报告。注意两点：① 下次 `docker compose up --build` 重建镜像会盖掉在线更新得到的版本，想固定某个 Release 仍用上一节的拉源码重建路径；② 旧版（`≤ v0.1.6`）在容器内会提示「不支持 / 请改用镜像升级」，那是旧版预期，不是要保留的行为。
